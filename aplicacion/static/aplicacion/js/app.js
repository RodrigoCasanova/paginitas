...
// //     let cantidadActual = parseInt(cantidadInput.value);  // Obtener la cantidad actual como entero
// //     cantidadActual++;
// //     cantidadInput.value = cantidadActual;  // Actualizar el valor del input
// //     actualizarTotalCarrito();
// // }
...
//     cantidadActual--;
//     if(cantidadActual >= 1){
//         cantidadInput.value = cantidadActual;  // Actualizar el valor del input solo si es mayor o igual a 1
//         actualizarTotalCarrito();
//     }
// }
...
document.addEventListener('DOMContentLoaded', function() {
    let carritoForm = document.getElementById('carrito-form');
    ...
    btnPagar.addEventListener('click', function(event) {
        ...
        for (let i = 0; i < carritoItems.length; i++) {
            ...
            let precioText = item.querySelector('.carrito-item-precio').textContent;
            let precio = parseFloat(precioText.replace('$', '').replace('.', '').replace(',', '.'));
            let cantidad = item.querySelector('.carrito-item-cantidad').value;
            carritoData.push({ titulo: titulo, precio: precio, cantidad: cantidad });
        }

        document.getElementById('carrito_data').value = JSON.stringify(carritoData);
        carritoForm.submit();
    });
});
